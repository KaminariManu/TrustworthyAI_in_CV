...
WaNet-specific parameters vs BadNet:
  --cover-rate  fraction of CLEAN cover samples added alongside poisoned ones (default: 0.1)

Poisoned-set directory naming (toolbox convention):
  poisoned_train_set/<dataset>/WaNet_<poison_rate>_cover=<cover_rate>[_poison_seed=<seed>]/

Checkpoints are saved to results/models/attack/WaNet/<run_dir>/
"""
...
def get_poison_set_dir(dataset: str, poison_rate: float, cover_rate: float) -> Path:
    """
    WaNet directory convention (mirrors supervisor.get_poison_set_dir):
      poisoned_train_set/<dataset>/WaNet_<poison_rate>_cover=<cover_rate>[_poison_seed=<seed>]
    The toolbox appends _poison_seed when config.record_poison_seed is True.
    """
    ratio = f'{poison_rate:.3f}'
    cr    = f'{cover_rate:.3f}'
    dir_name = f'WaNet_{ratio}_cover={cr}'
...
def create_poisoned_set_if_needed(args):
    poison_dir  = get_poison_set_dir(args.dataset, args.poison_rate, args.cover_rate)
    ...
    print(f"Creating WaNet poisoned set at: {poison_dir}")
    cmd = [
        sys.executable,
        ...
        '-cover_rate',  str(args.cover_rate),
    ]
...
def validate_asr(model, clean_loader, poison_transform, device, target_class):
    """ASR: fraction of non-target clean images predicted as target after WaNet warp."""
    model.eval()
    correct = total_n = 0
...
def train(args):
    ...
    exp_name  = (
        ...
        f"_WaNet_pr{args.poison_rate:.3f}_cr{args.cover_rate:.3f}"
        f"_{timestamp}"
    )
    ...
    # WaNet poison transform for ASR — supervisor reads identity_grid / noise_grid
    # from the poisoned set directory, so we pass args-like object via namespace
    target_class = _tb_config.target_class[args.dataset]
    try:
        # supervisor.get_poison_transform for WaNet calls get_poison_set_dir(args)
        # which returns a RELATIVE path (e.g. poisoned_train_set/cifar10/WaNet_...).
        # torch.load then resolves it against the cwd.  We must chdir to TOOLBOX_DIR
        # so the relative path resolves correctly before changing back.
    ...
    print(f"\n{Fore.CYAN}{'='*120}")
    print(f"Starting WaNet Attack Training: {args.epochs} epochs | patience={args.patience}")
    ...
    print(f"\n{Fore.GREEN}{'='*120}")
    print("WANET ATTACK TRAINING COMPLETE")
    print(f"  Best CA:      {best_acc:.2f}% (epoch {best_epoch})")
    print(f"  poison_rate={args.poison_rate}  cover_rate={args.cover_rate}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train model under WaNet backdoor attack',
    ...
    parser.add_argument('--poison-rate', type=float, default=0.05,
                        help='Fraction of training samples to poison (WaNet default: 0.05)')
    parser.add_argument('--cover-rate', type=float, default=0.1,
                        help='Fraction of clean cover samples (WaNet-specific, default: 0.1)')
