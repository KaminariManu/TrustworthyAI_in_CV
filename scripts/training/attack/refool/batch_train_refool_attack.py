"""
Batch trainer for Refool backdoor attack experiments.

Reads a named experiment group and runs train_refool_attack.py once for each
(model, dataset, pretrained) combination in that group.

Usage examples
--------------
# Run the default CIFAR-10 scratch group
python scripts/training/attack/refool/batch_train_refool_attack.py \\
    --experiment cifar10_scratch --poison-rate 0.1

# Run VGG models only on GTSRB from scratch
python scripts/training/attack/refool/batch_train_refool_attack.py \\
    --experiment gtsrb_scratch --models vgg16 vgg19
...
TRAIN_SCRIPT = Path(__file__).resolve().parent / 'train_refool_attack.py'
...
def run_experiment(cfg: dict, attack_args: argparse.Namespace, idx: int, total: int) -> bool:
    ...
    print(f"  attack=Refool  poison_rate={attack_args.poison_rate}")
    print(f"{bar}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch Refool attack training for an experiment group',
    ...
    print(f"\n{'='*80}")
    print(f"  Refool Batch Attack Training")
    ...
    for idx, cfg in enumerate(experiments, 1):
        ...
        if not success:
            failed.append(
                f"{cfg['model']}/{cfg['dataset']}/{'pretrained' if cfg.get('pretrained') else 'scratch'}"
            )
