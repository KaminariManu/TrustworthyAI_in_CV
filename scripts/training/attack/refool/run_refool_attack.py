"""
Master orchestrator for all Refool backdoor attack experiments.

Delegates to batch_train_refool_attack.py, one experiment group at a time.
...
Prerequisites
-------------
  VOCdevkit reflection images must exist under backdoor-toolbox-main/data/:
...
# CIFAR-10 only, 10%% poison rate
conda run -n backdoor-toolbox python scripts/training/attack/refool/run_refool_attack.py \\
    --mode 3 --poison-rate 0.10
...
BATCH_SCRIPT = Path(__file__).resolve().parent / 'batch_train_refool_attack.py'
...
def main():
    parser = argparse.ArgumentParser(
        description='Run all Refool backdoor attack experiments',
...
Examples:
  ...
  python run_refool_attack.py --mode 1

  # GTSRB only, 10%% poison rate
  python run_refool_attack.py --mode 4 --poison-rate 0.10

  # Preview everything without executing
  python run_refool_attack.py --mode 1 --dry-run
""",
    )
    ...
    bar = '=' * 80
    print(f"\n{bar}")
    print(f"  Refool Attack Master Orchestrator")
